...
Image cornerResponse(const Image &im, float k = 0.15f, float sigmaG = 1,
                     float factorSigma = 4);
vector<Point> HarrisCorners(const Image &im, float k = 0.15f, float sigmaG = 1,
...
Image autostitch(const Image &im1, const Image &im2, float blurDescriptor = 0.5,
                 float radiusDescriptor = 4);
...
Image visualizeCorners(const Image &im, const vector<Point> &pts, int rad = 2,
                       const vector<float> &color = vector<float>{1.0, 1.0,
                                                                  1.0});
